                       ...
                       double w0, double xi0, double xi1, const char *chm)
{
    ...
    fprintf(f, "xi a_over_ah e sig_e Vx sig_Vx Vy sig_Vy varpi sig_varpi t sig_t x sig_x y sig_y C_xx C_xy C_yy C_VxVx C_VxVy C_VyVy\n");
    ...
    for (int k = 0; k < sol->n; k++) {
        ...
        double C_VxVx, C_VxVy, C_VyVy;
        extract_sigma(yy + NS, ns_full, nn, sig);
        /* Velocity covariance block for Vx (state 1) and Vy (state 2). */
        extract_cov2(F, nn, 1, 2, &C_VxVx, &C_VxVy, &C_VyVy);
