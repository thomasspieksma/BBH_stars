...
// Computes all Y^R_lm(cos_theta, phi) for l = 0..l_max, m = -l..l.
// Stored in out[l*l + l + m].  Convention: Condon-Shortley phase absorbed into
// P_l^m; real SH defined as  sqrt(2)*K*P*cos(m*phi) (m>0),  K*P (m=0),
// sqrt(2)*K*P*sin(|m|*phi) (m<0).  These satisfy the addition theorem
// P_l(cos gamma) = (4pi/(2l+1)) * sum_m  Y_lm(n1)*Y_lm(n2).
void compute_real_Ylm(double cos_theta, double phi, int l_max, double* out)
{
    const double sin_theta = std::sqrt(std::max(0.0, 1.0 - cos_theta * cos_theta));
    const int sz = l_max + 1;

    // cos(m*phi) and sin(m*phi) via Chebyshev recurrence
    ...
    // Associated Legendre P_l^m(cos_theta) with CS phase
    ...
    for (int m = 0; m <= l_max; ++m)
        for (int l = m + 2; l <= l_max; ++l)
            P(l, m) = ((2*l - 1) * cos_theta * P(l-1, m) - (l + m - 1) * P(l-2, m)) / (double)(l - m);

    // Factorials (safe for l_max <= 20, max (l+m)! = 40! < 1e48)
...
int main(int argc, char* argv[]) {
    ...
    if (argc < 3)
    {
        std::cerr << "Usage: " << argv[0] << " q e [N_v] [N] [l_max]\n"
                  << "  l_max > 0 : write SH harmonics file only (compact)\n"
                  << "  l_max = 0 : write per-particle file only (default)\n"
    ...
    int N_v_arg = (argc >= 4) ? std::stoi(argv[3]) : 2000;
    int N       = (argc >= 5) ? std::stoi(argv[4]) : 10000;
    int l_max_arg = (argc >= 6) ? std::stoi(argv[5]) : 0;
    bool write_per_particle = (l_max_arg <= 0);
    bool write_harmonics    = (l_max_arg != 0);
    int  l_max = std::abs(l_max_arg);
    ...
    std::vector<double> v_values = v_list(q, N_v_arg); // compute list of velocities
    std::cout << "Parameters: q=" << q << ", e=" << e << ", N_v=" << v_values.size()
              << ", N=" << N << ", l_max=" << l_max
              << (write_harmonics ? " [harmonics]" : "")
              << (write_per_particle ? " [per-particle]" : "") << std::endl;
    ...
    // ── Per-particle binary output (original format) ────────────────────────
    ...
    // ── Spherical harmonic moments binary output ─────────────────────────
    if (write_harmonics)
    {
        std::ostringstream fname;
        ...
        if (!hout.is_open()) {
            ...
            // 7 quantities: dE, dvx, dvy, dvz, dLx, dLy, dLz
            constexpr int N_Q = 7;
            ...
            for (int iv = 0; iv < (int)v_values.size(); ++iv) {
                ...
                // Accumulators: first moments M^X_{lm} and second moments S^{X^2}_{lm}
                std::vector<double> M(N_Q * n_sh, 0.0);  // M[q*n_sh + lm]
                std::vector<double> S(N_Q * n_sh, 0.0);  // S[q*n_sh + lm]
                ...
                for (const auto &pair : all_results[iv]) {
                    ...
                    double X[N_Q] = {dE, dv[0], dv[1], dv[2], dL[0], dL[1], dL[2]};
                ...
                // Write 7 first-moment blocks then 7 second-moment blocks
                hout.write(reinterpret_cast<const char*>(M.data()), M.size() * sizeof(double));
                hout.write(reinterpret_cast<const char*>(S.data()), S.size() * sizeof(double));
            }

            double file_mb = (44.0 + v_values.size() * (12.0 + 2 * N_Q * n_sh * 8.0)) / (1024*1024);
            std::cout << "Harmonics file written: " << fname.str()
                      << " (l_max=" << l_max << ", ~" << std::fixed
                      << std::setprecision(1) << file_mb << " MB)" << std::endl;
        }
    }

    // Write text summary files for each TmaxCut (same as original code)
